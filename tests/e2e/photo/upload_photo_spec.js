...
describe('User visits photo upload page', function() {
  ...
  afterEach(function() {
    ...
    Photo.remove({}, function (err) { if (err) return handleError(err) });
  });
  ...
  describe('Upload Photo', function(){
    ...
    before(function(done) {
      browser.visit('/photo/upload', function() {
        browser.attach('photo-file-name', __dirname + "/arnold.jpg")
        .fill('photo_name', 'test_photo')
        // Front end disabled button until file uploaded / need to find workaround / mock
